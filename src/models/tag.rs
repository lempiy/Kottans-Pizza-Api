...
impl Tag {
    ...
    pub fn get_records_count(db: &MutexGuard<Connection>) -> Result<i64> {
        match db.query("SELECT get_count($1);", &[&"tag"]) {
            Ok(query) => {
                for row in query.iter() {
    ...
    pub fn validate_tags_exist(
        ...
        if !has_unique_elements(tag_ids) {
            return Err(ValidationError {
                ...
                message: Some(Cow::from("Tags array has duplicate ids")),
                params: HashMap::new(),
            });
        };
        let mut query = tag_ids.iter().enumerate().fold(
            "SELECT id FROM tag WHERE id IN (".to_string(),
            |acc, x| {
                let (i, _) = x;
                acc + &format!("${},", i + 1)
            },
        ...
        match db.query(&query, &ids) {
            Ok(query) => if query.len() == tag_ids.len() {
                Ok(())
            } else {
