use redis::{PubSub, Commands, Connection};
...
impl Manager {
    ...
    pub fn subscribe(&self, channel_name: String) -> Receiver<String> {
        ...
        let _ = self.send_channel.send(Action::Subscribe(channel_name.clone()));
        let subs = vec![tx];
